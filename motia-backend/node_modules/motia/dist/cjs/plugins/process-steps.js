...
const collect_plugin_steps_1 = require("./collect-plugin-steps");
const processSteps = async (motiaServer, plugins, baseDir) => {
    ...
    for (const plugin of plugins) {
        if (plugin.dirname && plugin.steps) {
            ...
            try {
                const pluginSteps = await (0, collect_plugin_steps_1.collectPluginSteps)(plugin.dirname, plugin.steps, baseDir, printer);
                const version = `plugin_${(0, node_crypto_1.randomUUID)()}:${Math.floor(Date.now() / 1000)}`;
                for (const { filePath, config } of pluginSteps) {
                    try {
                        const isCreated = motia.lockedData.createStep({ filePath, version, config: { ...config, flows: [core_1.PLUGIN_FLOW_ID] } }, { disableTypeCreation: true });
                        if (isCreated) {
