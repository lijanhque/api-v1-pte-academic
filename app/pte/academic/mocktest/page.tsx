...
async function getSectionalTests() {
  ...
  // The user said "sectional tests", let's look for tests with a section.
  // Or maybe testType='sectional' if I add it.
  // Based on schema comment: // 'mock', 'practice', 'scored'
  // I'll assume 'practice' with a section is a sectional test, or maybe 'mock' with a section.
  // Let's fetch all and filter in UI or fetch separately.
  // Let's assume 'mock' tests with a section are sectional mocks, and 'mock' without section are full mocks.
  return await db.select().from(pteTests).where(eq(pteTests.testType, 'mock'))
}
    ...
    <div className="container mx-auto p-6 space-y-8">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground">
          Take full-length or sectional mock tests to evaluate your readiness.
        </p>
      </div>
                        ...
                        <Badge
                          variant={
                            attempt.status === 'completed'
                              ? 'default' // 'success' is not a standard variant, using default
